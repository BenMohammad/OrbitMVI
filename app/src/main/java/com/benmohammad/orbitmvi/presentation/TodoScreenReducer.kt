...
import com.benmohammad.orbitmvi.domain.todo.TodoStatus
import com.benmohammad.orbitmvi.domain.user.UserProfileStatus
...
class TodoScreenReducer {
    ...
    internal fun reduceLoadTodos(
        ...
        return when(event) {
            ...
            is TodoStatus.Result -> currentState.copy(screenState = ScreenState.Ready, todoList = event.todoList)
        }
    }
    ...
    internal fun reduceLoadUserProfileSwitch(
        ...
        return when (event.userProfileSwitchStatus) {
